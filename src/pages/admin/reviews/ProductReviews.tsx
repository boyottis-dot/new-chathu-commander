...
import { productReviews as initialReviews, ProductReview, ReviewStatus } from "@/lib/mock-data-reviews";
...
export default function ProductReviews() {
  ...
  const filtered = reviews.filter((r) => {
    const matchesSearch = r.productName.toLowerCase().includes(search.toLowerCase()) || r.customerName.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search by product or customer..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" /></div>
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Customer</TableHead><TableHead>Rating</TableHead><TableHead className="hidden md:table-cell">Comment</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {filtered.map((r) => (
                <TableRow key={r.id} className={r.flagged ? "bg-destructive/5" : ""}>
                  <TableCell><div><p className="font-medium text-foreground">{r.productName}</p><p className="text-xs text-muted-foreground">{r.vendorName}</p></div></TableCell>
